...
def parse_msrc_document(document: Dict[str, object], start: date, end: date, target_products: Sequence[str], raw_hash: str) -> List[Event]:
    ...
    for vulnerability in document.get("Vulnerability", []) or []:
        ...
        event = Event(
            ...
            recommended_action="Prioritize patch applicability review; test the linked security update on representative guest and host images.",
        ...
        if block.tag == "h2":
            ...
            if body and _section_date_match(current_month, body, start, end):
                ...
                events.append(Event(
                    ...
                    summary=body[:600],
                    evidence=body[:1000],
                    recommended_action="Review the originating KB, affected platforms, workaround, and resolving update; validate before image rollout.",
...
def parse_lifecycle(html: str, source_id: str, url: str, product: str, start: date, end: date, raw_hash: str) -> List[Event]:
    ...
    for row in parse_table_rows(html):
        ...
        events.append(Event(
            ...
            summary=text[:700],
            evidence=text[:1000],
            recommended_action="Compare this milestone with the internal image inventory and schedule upgrade or retirement work where applicable.",
...
def parse_insider_sitemap(xml_body: bytes, start: date, end: date, raw_hash: str) -> List[Event]:
    ...
    for url, modified in _xml_locs(xml_body):
        ...
        events.append(Event(
            ...
            roles=infer_roles(products, components),
            components=components,
            summary="Discovered from the official Windows Insider sitemap. Article-body retrieval requires browser enrichment.",
            evidence=f"Official sitemap entry; last modified {modified or 'not provided'}.",
            recommended_action="Treat as preview only; enrich and review if the title matches an internal cloud-desktop component.",
